	...
	{if $amIgod}		
		...
		{if $user_logged_in neq $userdata[nr].user_login && $userdata[nr].user_id neq '1'}
			{if $userdata[nr].user_enabled}
				<a class="btn" href="?mode=disable&user={$userdata[nr].user_login}"><img src="{$my_base_url}{$my_pligg_base}/templates/admin/images/user_delete.gif.gif" align="absmiddle"/> {#PLIGG_Visual_View_User_Disable#}</a>
			{else}
				<a class="btn" href="?mode=enable&user={$userdata[nr].user_login}"><img src="{$my_base_url}{$my_pligg_base}/templates/admin/images/user_add.gif" align="absmiddle"/> {#PLIGG_Visual_View_User_Enable#}</a>
			{/if}
