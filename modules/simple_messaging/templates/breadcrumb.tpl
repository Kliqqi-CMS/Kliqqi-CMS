...
<li>{if $modulename_sm neq "simple_messaging_inbox"}<a href="{$URL_simple_messaging_inbox}">{/if}{#PLIGG_MESSAGING_Inbox#}{if $modulename_sm neq "simple_messaging_inbox"}</a> <span class="divider">/</span>{/if}</li>
...
{if $modulename_sm eq "simple_messaging_viewsentmsg"}<li><a href="{$my_pligg_base}/module.php?module=simple_messaging&view=sent">{#PLIGG_MESSAGING_Sent#}</a> <span class="divider">/</span></li><li>{#PLIGG_MESSAGING_Message#}</li>{/if}
